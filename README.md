...
### Live dashboard
Cards for every host with CPU, RAM and disk bars, uptime, OS version, top processes, and a Docker badge showing how many containers are up. When Docker is present, the card lists **running containers and the ports they occupy**. Bars turn yellow, then red, so a sick server is obvious from across the room.
...
Then run `omny`. The interface defaults to Simplified Chinese (`omny --lang en` for English). Press `a` to add a host, `/` to search, `?` for help, `Shift+K` to set up keys on the selected host. On a host with Docker, the detail view (`Enter` on a card, then `4`) lists running containers and the ports they occupy.
