...
{#if $keySetup}
  ...
  <Modal label={$t.keySetup.title} onClose={dismissKeySetup}>
    <div class="space-y-4 px-5 py-4">
      <div class="flex items-center gap-2.5">
        <Icon name="key" size={16} />
        <h2 class="min-w-0 truncate text-sm font-semibold">{interpolate($t.keySetup.title, { name: run.hostName })}</h2>
      </div>
      ...
      {:else if phase.kind === 'failed'}
        <div class="flex items-start gap-2.5">
          ...
          <div class="min-w-0 space-y-1">
            ...
            <p class="text-xs text-faint">{$t.keySetup.passwordUnchanged}</p>
          </div>
        </div>
        <div class="flex justify-end">
      ...
      {:else}
        <div class="flex items-start gap-2.5">
          ...
          <div class="min-w-0 space-y-1">
            ...
            <p class="text-xs text-faint">{$t.keySetup.passwordRestored}</p>
          </div>
        </div>
        <div class="flex justify-end">
          <Button variant="ghost" onclick={dismissKeySetup}>{$t.keySetup.close}</Button>
